...
class ApiHandler {
   ...
   */
  getHeaderPagination(response: APIResponse): string | undefined {
    // Use the public headers() API instead of internal properties
    const headers = response.headers();
    const linkHeaderValue = headers['link'] ?? headers['Link'];

    if (typeof linkHeaderValue === 'string' && linkHeaderValue.includes('rel="next"')) {
      const m = linkHeaderValue.match(/<([^>]+)>;\s*rel=\"next\"/);
