...
test.describe('Login tests', () => {
  ...
  test('User log in invalid password', async ({ page }) => {
    ...
    await test.step('Validate that correct error messages appear', async () => {
      await expect(mainPage.elements.loginUnsuccesfulErrorLabel()).toHaveText(
        errorMessage.loginUnsuccesfulErrorMessage,
      );
  ...
  test('User log in invalid email', async ({ page }) => {
    ...
    await test.step('Validate that correct error messages appear', async () => {
      await expect(mainPage.elements.enterValidEmailErrorLabel()).toHaveText(
        errorMessage.invalidEmailLoginErrorMessage,
      );
    });
  });
